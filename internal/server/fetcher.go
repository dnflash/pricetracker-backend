	...
	for _, i := range is {
		...
		case siteShopee:
			...
			if shopeeItem.Price != i.Price || shopeeItem.Stock != i.Stock {
				s.Logger.Debugf("fetchData: Value of price or stock changed, updating Item: %s, ID: %s", itemName, i.ID.Hex())
			...
			if shopeeItem.Price != i.Price {
				...
				fcmReq := client.FCMSendRequest{
					Notification: client.FCMNotification{
						...
						Body:        fmt.Sprintf("%s is now Rp. %d", itemName, shopeeItem.Price),
